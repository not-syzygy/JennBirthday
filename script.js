...
function updateText(newStrings) {
    opts.strings = newStrings;
    letters = [];
    for (var i = 0; i < opts.strings.length; i++) {
        for (var j = 0; j < opts.strings[i].length; j++) {
            letters.push(new Letter(opts.strings[i][j], 
                opts.cx + j * opts.charSpacing - opts.strings[i].length * opts.charSpacing / 2,
                opts.cy + i * opts.lineHeight - opts.strings.length * opts.lineHeight / 2));
        }
    }
}
...
function updateTextWithDelay(textArray, delay, callback) {
	// Function to update text with a delay
    setTimeout(() => {
